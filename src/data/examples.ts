...
{
    id:"Nim-Spiel mit Smartem Computer",
    title: "Nim-Spiel mit Smartem Computer",
    icon: "🎮",
    description: "Ein verbessertes Nim-Spiel, bei dem der Computer eine intelligente Strategie verwendet, um zu gewinnen.",
    tip: "Der Computer sollte versuchen, die Anzahl der Stöcke so zu reduzieren, dass sie ein Vielfaches von 4 erreicht, um eine Gewinnposition zu schaffen.",
    code: `#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
void zeigeStoecke(int anzahl) {
    for (int i = 0; i < anzahl; i++) printf("|");
    printf("\n\n");
}

int computerZug(int stock, int stufe) {
    if (stock == 0) return 0;

    
    if (stufe == 1) {
        int z = rand() % 3 + 1;
        return (z > stock) ? stock : z;
    }

    
    if (stufe == 2) {
        if (stock <= 3) return stock;
        int z = rand() % 3 + 1;
        return (z > stock) ? stock : z;
    }
...
int main() {
    int stock = 13;
    int eingabe, computer;
    int spielerAmZug = 1;
    int schwierigkeit = 0;

    srand(time(NULL));

    printf("=== Nim - Stöckchen ziehen ===\n");
    printf("Start mit %d Stöcken\n\n", stock);

    printf("Schwierigkeitsstufe wählen (1–4):\n");
    printf("  1 = Zufall\n");
    printf("  2 = Gewinnt sofort wenn möglich\n");
    printf("  3 = Gewinnt sofort oder blockt nächsten Gewinn\n");
    printf("  4 = Optimal (fast nicht zu schlagen)\n\n");

    do {
        printf("Auswahl (1-4): ");
        scanf("%d", &schwierigkeit);
    } while (schwierigkeit < 1 || schwierigkeit > 4);
    ...
    while (stock > 0) {
        if (spielerAmZug) {
            ...
            if (eingabe < 1 || eingabe > 3 || eingabe > stock) {
                printf("Ungültige Eingabe! (1–3 und <= restliche Stöcke)\n\n");
                continue;
            }

            stock = minus(stock, eingabe);
            printf("Du nimmst %d.\n", eingabe);
        }
        else {
            computer = computerZug(stock, schwierigkeit);
            stock = minus(stock, computer);
            printf("Computer nimmt %d.\n", computer);
        }
        zeigeStoecke(stock);
        spielerAmZug = !spielerAmZug;
    }
